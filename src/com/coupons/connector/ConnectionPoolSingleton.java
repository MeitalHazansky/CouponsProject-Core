...
public class ConnectionPoolSingleton {
	...
	private ConnectionPoolSingleton(){
		// Initialize the Connection Pool with a list of Connections up to the MAX number of connections
		try {
	...
	public synchronized Connection getConnection() throws ConnectionException {
		...
		// waiting if the connection pool is empty
			...
			throw new ConnectionException("PoolSingleton: An Error occured while trying to GET a connection: " + e.getMessage());
		}
	}
	...
	public synchronized void returnConnection(Connection connection) throws ConnectionException {
		...
		try {
			...
			throw new ConnectionException("An Error occured while trying to RETURN a connection: " + e.getMessage());
		}
	}
	 ...
	 */
	public static void closeAllConnections() throws ConnectionException {
		...
		try {
			...
			throw new ConnectionException("PoolSingleton: An Error occured while trying to CLOSE ALL connections: " + e.getMessage());
		}
	}
	...
	private Connection createConnection() throws ConnectionException {
		try	{
			...
			connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/coupons?user=root&password=1234&useSSL=false");
			return connection;
		}
		catch (Exception e){
			throw new ConnectionException("PoolSingleton: An Error occured while trying to CREATE a connection: " + e.getMessage());
		}
	}
